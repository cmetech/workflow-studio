...
for (const viewport of [
  ...
  test(`loop References preserve canvas space and visible Problems counts at ${viewport.width}x${viewport.height}`, async ({
    ...
    const layerTabs = page.getByRole('tablist', { name: 'Validation layers' })
    await expect(layerTabs.getByRole('tab', { name: 'Syntax 0' })).toBeVisible()
    await expect(layerTabs.getByRole('tab', { name: 'Contract 0' })).toBeVisible()
    await expect(layerTabs.getByRole('tab', { name: 'Semantic 0' })).toBeVisible()
    await expect(layerTabs.getByRole('tab', { name: 'Compatibility 0' })).toBeVisible()
    const operational = layerTabs.getByRole('tab', { name: 'Operational 20' })
    await expect(operational).toHaveAttribute('aria-selected', 'true')
    await expect(page.getByText('Deterministic state advisory 1.')).toBeVisible()
    const syntax = layerTabs.getByRole('tab', { name: 'Syntax 0' })
    await syntax.click()
