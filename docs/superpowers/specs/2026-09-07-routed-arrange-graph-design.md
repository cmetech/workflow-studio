...
The implemented worker uses the pinned package's supported `elk-api.js` with a local endpoint factory. Following the approved Windows stabilization amendment, `src/workers/elk-engine-worker.ts` imports the official `elk-worker.min.js` dispatcher into the existing dedicated layout worker rather than spawning a descendant worker: packaged Windows WebView2 can stall while fetching nested worker scripts. A narrow endpoint adapter keeps ELK's internal request/reply messages separate from application envelopes and retains the native publisher for final bounded results. Request validation, the bounded ordering/routing passes, normalization, retry, and complete-result validation all remain off the renderer thread in that one terminable worker. No environment spoofing, algorithm replacement, CSP relaxation, or timeout increase is used. Real emitted-worker tests exercise Chromium and WebKit with nested worker creation disabled; Chromium also verifies worker termination. The algorithm and configuration remain bundled locally.
