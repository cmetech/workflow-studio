...
describe('E2E perceptual performance policy', () => {
  ...
  it('keeps functional Windows coverage separate from enforced reference performance', () => {
    ...
    expect(config).toContain("process.env.WORKFLOW_STUDIO_E2E_REUSE_SERVER === 'true'")
    expect(config).toContain('reuseExistingServer: reuseExistingE2EServer')
    expect(globalSetup).toContain('const WARMUP_ATTEMPTS = 2')
    expect(globalSetup).toContain('await retryE2eWarmup(() => warmEntryGraphAttempt(context, baseURL))')
