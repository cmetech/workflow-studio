...
describe('release asset verification', () => {
  ...
  it.each([
    ['darwin', 'aarch64', 'LOOP24-Workflow-Studio_1.0.7_macos_aarch64.dmg'],
    ['darwin', 'x86_64', 'LOOP24-Workflow-Studio_1.0.7_macos_x86_64.dmg'],
    ['windows', 'x86_64', 'LOOP24-Workflow-Studio_1.0.7_windows_x86_64-setup.exe'],
  ...
  it('accepts exactly the v1.0.7 macOS and Windows public inventory', () => {
  ...
  it('normalizes Tauri Action API URLs to public exact-tag asset URLs without altering signatures', () => {
    ...
    expect(normalized.platforms['darwin-aarch64-app']).toEqual({
      url: 'https://github.com/cmetech/workflow-studio/releases/download/v1.0.7/LOOP24-Workflow-Studio_1.0.7_macos_aarch64.app.tar.gz',
      signature: 'signed-darwin-arm',
    })
    expect(normalized.platforms['windows-x86_64'].url).toContain(
      '/v1.0.7/LOOP24-Workflow-Studio_1.0.7_windows_x86_64-setup.exe',
    )
  ...
  it.each([
    'Other-Workflow-Studio_1.0.7_linux_x86_64.AppImage',
    'LOOP24-Workflow-Studio_9.9.9_linux_x86_64.AppImage',
    '../LOOP24-Workflow-Studio_1.0.7_linux_x86_64.AppImage',
    'LOOP24-Workflow-Studio_1.0.7_linux_x86_64/app.AppImage',
    'loop24-workflow-studio_1.0.7_linux_x86_64.AppImage',
    'LOOP24-Workflow-Studio_1.0.7_linux_x86_64.AppImage\nsecond',
  ])('rejects a wrong-brand/version or unsafe asset name: %s', (name) => {
    const invalid = manifestWith((copy) => {
      const existing = fixtureAsset(copy, 'LOOP24-Workflow-Studio_1.0.7_windows_x86_64-setup.exe')
  ...
  it('rejects case-folded collisions and duplicate installers', () => {
    const invalid = manifestWith((copy) => {
      const installer = fixtureAsset(copy, 'LOOP24-Workflow-Studio_1.0.7_windows_x86_64-setup.exe')
  ...
  it.each([
    'LOOP24-Workflow-Studio_1.0.7_linux_x86_64.AppImage',
    'LOOP24-Workflow-Studio_1.0.7_linux_x86_64.deb',
    'LOOP24-Workflow-Studio_1.0.7_linux_x86_64.rpm',
    'LOOP24-Workflow-Studio_1.0.7_windows_x86_64-setup.nsis.zip',
  ...
  it('rejects stale Linux updater keys', () => {
    const invalid = manifestWith((copy) => {
      copy.updater.platforms['linux-x86_64'] = {
        url: 'https://github.com/cmetech/workflow-studio/releases/download/v1.0.7/LOOP24-Workflow-Studio_1.0.7_linux_x86_64.AppImage.tar.gz',
  ...
  it('rejects missing checksums, signature companions, updater targets, and empty bytes', () => {
    const noChecksum = manifestWith((copy) => {
      fixtureAsset(copy, 'LOOP24-Workflow-Studio_1.0.7_macos_aarch64.dmg').sha256 = undefined
    })
    ...
    const zeroBytes = manifestWith((copy) => {
      fixtureAsset(copy, 'LOOP24-Workflow-Studio_1.0.7_macos_aarch64.dmg').size = 0
    })
  ...
  it('rejects updater URLs for unknown or cross-release assets', () => {
    const invalid = manifestWith((copy) => {
      copy.updater.platforms['windows-x86_64']!.url =
        'https://github.com/cmetech/workflow-studio/releases/download/v1.0.7/not-uploaded.tar.gz'
    })
  ...
  it('separates updater normalization from checksumming published bytes', () => {
    ...
    try {
      ...
      expect(unpublished.stderr).toContain('exact v1.0.7 release')
      ...
      expect(readFileSync(join(directory, 'latest.json'), 'utf8')).toContain('/releases/download/v1.0.7/')
  ...
  it('rejects a stale Linux artifact before updater normalization or checksum generation', () => {
    ...
    try {
      writeFileSync(join(directory, 'LOOP24-Workflow-Studio_1.0.7_linux_x86_64.AppImage'), 'stale')
  ...
  it('rejects updater bytes with an invalid cryptographic signature before checksumming', () => {
    const { root, directory, tauriConfig } = materializeCryptoRelease()
    try {
      writeFileSync(join(directory, 'LOOP24-Workflow-Studio_1.0.7_macos_aarch64.app.tar.gz'), 'Test')
      const result = spawnSync(
        process.execPath,
        [
          'scripts/verify-release-assets.mjs',
...
function draftLifecycleRelease(overrides: Partial<DraftLifecycleRelease> = {}): DraftLifecycleRelease {
  return {
    ...
    draft: true,
    prerelease: false,
    name: 'LOOP24 Workflow Studio v1.0.7',
...
describe('release workflow contract', () => {
  ...
  it('creates the v1.0.7 draft only after two clean absence reads', () => {
  ...
  it.each([
    [
      ...
      /expected exactly one release tagged v1\.0\.7; found 2/i,
    ],
...
npm run examples:check`)
  ...
  it('ties native Tauri v2 updater output and matrix bundles to the exact public inventory', () => {
    ...
    const tauri = jobSteps('build').find((step) => step.id === 'tauri')
    const updaterName = 'LOOP24-Workflow-Studio_1.0.7_windows_x86_64-setup.exe'
    ...
    expect(fixture.assets.map((asset) => asset.name)).toEqual([
      'LOOP24-Workflow-Studio_1.0.7_macos_aarch64.dmg',
      'LOOP24-Workflow-Studio_1.0.7_macos_aarch64.app.tar.gz',
      'LOOP24-Workflow-Studio_1.0.7_macos_aarch64.app.tar.gz.sig',
      'LOOP24-Workflow-Studio_1.0.7_macos_x86_64.dmg',
      'LOOP24-Workflow-Studio_1.0.7_macos_x86_64.app.tar.gz',
      'LOOP24-Workflow-Studio_1.0.7_macos_x86_64.app.tar.gz.sig',
      updaterName,
      `${updaterName}.sig`,
      'latest.json',
      'SHA256SUMS',
    ...
    const windowsUrl = `https://github.com/cmetech/workflow-studio/releases/download/v1.0.7/${updaterName}`
  ...
  it('selects workflow-studio.exe from an NSIS payload that also contains uninstall.exe', () => {
    ...
    writeFileSync(join(bundleDirectory, 'workflow-studio_1.0.7_x64-setup.exe'), 'fixture')
