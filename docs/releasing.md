...
3. Complete the v3.1.0 release verification record and prepare the Windows clean-machine acceptance record for post-publication follow-up.
...
4. Create an annotated `v3.1.0` tag on a commit contained in `origin/base`, then push that exact tag.
...
## v3.0.1 published evidence

The v3.0.1 patch release corrects Windows native state persistence for Arrange Graph layout, active branding, setup readiness, and updater preferences. The annotated tag peels to exact release commit `7608947b4d17e49cd064fc978d330c76aab8c281`. Protected CI run `34406142397` passed all five jobs, and protected release workflow run `34408692126` built and verified all native packages and the exact ten-asset draft. GitHub release `385868225` was published as the latest release on 2026-09-09. Exact hashes and post-publication Windows UAT are recorded in the v3.0.1 acceptance document.

## v3.0.2 published evidence

The Windows-primary stabilization release is published as latest at
`https://github.com/cmetech/workflow-studio/releases/tag/v3.0.2` (release
`392944606`, September 21, 2026, 15:51:15 UTC). Immutable application tag
`af61f1e7c54e23d086b6e0a8b7adc8a6b3d575b4` passed exact-tag CI. Separately pinned
release tooling `7413cd287331ec20c34839b5d67a0022387ff9fa` includes test-first
renderer-build prerequisite and fail-fast Bash corrections. Full tooling CI
`35610262674` and `35611348944`, all three native jobs and final draft verification
in `35611353267`, and independent downloaded-artifact/payload verification passed
before publication. Anonymous public installer, updater and checksum bytes were
