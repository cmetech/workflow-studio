...
The v3.1.0 workflow package authoring release is being prepared. It adds complete packages, integrated code/resource editors, and marketplace-compatible local preparation. See the [v3.1.0 acceptance record](verification/version-3.1.0-release-acceptance.md) for build status and boundaries. The latest published version remains v3.0.3 until v3.1.0 passes release verification and is published. The verified installer commands below always select the latest published release.

The Workflow Studio v3.0.2 Windows-primary stabilization release improves native file operations, background process launching, folder/Git discovery, canvas interaction, packaged Arrange Graph and keyboard activation. It retains the v3.0.1 persistence hotfix and complete v3.0.0 feature set. See the [v3.0.2 acceptance record](verification/version-3.0.2-release-acceptance.md) for publication status and accepted performance limitations. The v1.0.7 documentation-and-shortcuts draft and v2.0.0 verified unpublished draft remain historical records. The v1.0.8 loop-group visual-authoring candidate was superseded without a tag or release. The v2.0.1 UI customization recovery candidate was superseded without a tag or release. Native installers are distributed through the public `cmetech/workflow-studio` GitHub Releases page. The application does not have an Apple Developer ID signature, Apple notarization, or a Microsoft Authenticode signature. Gatekeeper or SmartScreen warnings are expected. The release workflow produces `SHA256SUMS`, and the in-app updater uses a separate first-party cryptographic signature to prevent modified update artifacts from installing.
...
Windows ARM64 is unsupported and is not silently redirected to x64. Linux is deferred and unsupported by the bootstrap; the v3.0.2 release matrix has no Linux AppImage, package, updater entry, or installation path. Linux CI coverage does not imply installer availability.

## One-line verified installer launch

These commands load the immutable v1.0.5 bootstrap scripts, which install the latest published release. The bootstrap tag identifies the reviewed script bytes, not the application version it downloads. They select the latest published release, not an unpublished draft. Close Workflow Studio and save your work before installing or upgrading. Review [the shell script](../scripts/install.sh) or [the PowerShell script](../scripts/install.ps1) first if that is your policy.
...
Linux packaging is deferred. The immutable v1.0.5 bootstrap does not support Linux, and v3.0.2 has no Linux fallback artifact.
