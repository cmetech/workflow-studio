...
describe('authenticated release-list resolution', () => {
  ...
  it('uses a distinct no-match status so workflow fallback cannot mask malformed release state', () => {
    ...
    try {
      ...
      expect(invocation.result.stderr).toMatch(/exactly one release tagged v1\.0\.8; found 0/i)
  ...
  it.each([
    [
      ...
      /exactly one release tagged v1\.0\.8; found 2/i,
    ],
  ...
  it('requires complete absence before a draft may be created', () => {
    ...
    try {
      ...
      expect(existing.result.stderr).toMatch(/expected no release tagged v1\.0\.8; found 1/i)
    } finally {
  ...
  it('treats the tag as data and never executes shell syntax', () => {
    ...
    const tag = `v1.0.8; touch ${sentinel}`
